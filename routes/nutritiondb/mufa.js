...
router.post("/", async (req, res) => {
  console.log('here in checking');
    console.log("Post all mufa API called", req.body);

    res.json(req.body);

    var resp_data = await new_nutrition_helper.insert_mufa(
     req.body
    );
    if (resp_data.status == 0) {
      logger.error("Error occured while mufa = ", resp_data);
      res.status(config.INTERNAL_SERVER_ERROR).json(resp_data);
    } else {
      logger.trace("mufa save successfully = ", resp_data);
