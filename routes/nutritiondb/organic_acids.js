...
router.post("/", async (req, res) => {
    console.log("Post all organic_acids API called");
    
    var resp_data = await new_nutrition_helper.insert_organic_acids(
     req.body
    );
    if (resp_data.status == 0) {
      logger.error("Error occured while adding organic_acids ", resp_data);
      res.status(config.INTERNAL_SERVER_ERROR).json(resp_data);
    } else {
      logger.trace("organic_acids save successfully = ", resp_data);
