...
router.post("/", async (req, res) => {
    console.log("Post all vitamins API called");
    var resp_data = await new_nutrition_helper.insert_vitamins(
     req.body
    );
    if (resp_data.status == 0) {
      logger.error("Error occured while adding vitamins = ", resp_data);
      res.status(config.INTERNAL_SERVER_ERROR).json(resp_data);
    } else {
      logger.trace("vitamins save successfully = ", resp_data);
