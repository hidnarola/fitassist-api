...
/**
 * @api {get} /user/user_post Get all
 * @apiName Get all
 * @apiGroup User Post
 *
...
router.get("/", async (req, res) => {
  ...
  if (resp_data.status == 0) {
    logger.error(
      "Error occured while fetching get all user post photos = ",
      resp_data
    );
    res.status(config.INTERNAL_SERVER_ERROR).json(resp_data);
  } else {
    logger.trace("user post photos got successfully = ", resp_data);
    res.status(config.OK_STATUS).json(resp_data);
  }
});

/**
 * @api {get} /user/user_post/:user_post_id Get by ID
 * @apiName Get by ID
 * @apiGroup User Post
 *
 * @apiHeader {String}  x-access-token user's unique access-key
...
/**
 * @api {put} /user/user_post/:photo_id Update
 * @apiName Update
 * @apiGroup User Post
 *
 * @apiHeader {String}  Content-Type application/json
...
/**
 * @api {delete} /user/user_post/:photo_id Delete
 * @apiName Delete
 * @apiGroup User Post
 *
 * @apiHeader {String}  x-access-token user's unique access-key
