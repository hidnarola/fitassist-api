...
router.post("/", async (req, res) => {
    ...
    if (!errors) {
        ...
        if (measurement_unit_data.status === 0) {
            logger.error("Error while inserting measurement data = ", measurement_unit_data);
            measurement_obj = {
                userId: authUserId,
                logDate: logDate,
                neck: req.body.neck,
            ...
            var neck = await common_helper.unit_converter(req.body.neck, bodyMeasurement);
            var shoulders = await common_helper.unit_converter(req.body.shoulders, bodyMeasurement);
            var chest = await common_helper.unit_converter(req.body.chest, bodyMeasurement);
            var upperArm = await common_helper.unit_converter(req.body.upperArm, bodyMeasurement);
            var waist = await common_helper.unit_converter(req.body.waist, bodyMeasurement);
            var forearm = await common_helper.unit_converter(req.body.forearm, bodyMeasurement);
            var hips = await common_helper.unit_converter(req.body.hips, bodyMeasurement);
            var thigh = await common_helper.unit_converter(req.body.thigh, bodyMeasurement);
