 ...
 *          status 2 - If user's progress photos not found, with appropriate message
 */
user_progress_photo_helper.get_user_progress_photos = async (search_obj, start, limit, sort) => {
    try {
 ...
 */
user_progress_photo_helper.get_user_progress_photos_month_wise = async (search_obj, limit) => {
    try {
...
user_progress_photo_helper.get_user_progress_photo_by_id = async id => {
    try {
        ...
        if (user_progress_photos && user_progress_photos.length !== 0 && user_progress_photos[0]) {
            return {
                status: 1,
                message: "User progress photos found",
...
user_progress_photo_helper.get_first_and_last_user_progress_photos = async id => {
    try {
        ...
        if (user_progress_photos && user_progress_photos.length !== 0) {
            return {status: 1, message: "User progress photos found", user_progress_photos: user_progress_photos[0]};
        } else {
            return {status: 2, message: "No user progress photos available"};
        }
    } catch (err) {
        return {status: 0, message: "Error occured while finding user progress photos", error: err};
    }
};
...
user_progress_photo_helper.insert_user_progress = async user_progress_obj => {
    ...
    try {
        ...
        return {status: 1, message: "user progress inserted", user_progress_photo: user_progress_data};
    } catch (err) {
        return {status: 0, message: "Error occured while inserting user progress photo", error: err};
    }
};
...
user_progress_photo_helper.insert_user_progress_photo = async user_progress_photo_obj => {
    try {
        let user_progress_photo = await UserProgressPhotos.insertMany(user_progress_photo_obj);
        return {status: 1, message: "user progress photos inserted", user_progress_photo: user_progress_photo};
    } catch (err) {
        return {status: 0, message: "Error occured while inserting user progress photo", error: err};
    }
};
...
user_progress_photo_helper.delete_user_progress_photo = async (id) => {
    try {
        ...
        if (userProgressPhoto) {
            let userProgressId = userProgressPhoto.progressId ? userProgressPhoto.progressId : null;
            let deleteResource = await UserProgressPhotos.deleteOne(userProgressPhoto);
            let remainingProgressPhotos = await UserProgressPhotos.count({progressId: userProgressId});
            if (typeof remainingProgressPhotos === 'undefined' || remainingProgressPhotos <= 0) {
                await UserProgress.deleteOne({_id: userProgressId});
            }
            let responseObj = {status: 1, message: "Record has been deleted", user_progress_photo: deleteResource};
            return responseObj;
        } else {
        ...
        return {status: 0, message: "Error occured while deleting user progress photo", error: err};
    }
};
