var Programs_rating = require("./../models/programs_rating");
var programs_rating_helper = {};
/**
 ...
 *                 status -> 0 = error
 */
programs_rating_helper.save_program_rating = async (program_rating_obj) => {
    try {
        let selectCondition = {
        ...
        if (result && result.status && result.status === 1) {
            ...
            if (prevData && prevData.length > 0) {
                ...
                newData = await Programs_rating.findOneAndUpdate(condition, program_rating_obj, {new : true});
            } else {
                let newObj = new Programs_rating(program_rating_obj);
...
async function find_programs_ratings(condition) {
    try {
        let result = await Programs_rating.find(condition);
        return {
            status: 1,
            message: "Success",
            data: result
...
module.exports = programs_rating_helper;