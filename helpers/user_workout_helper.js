...
user_workout_helper.get_user_workout = async (userId) => {
    try {
        var user_workout = await UserWorkout.find(userId);
        if (user_workout) {
            return { "status": 1, "message": "user workouts details found", "user_workouts": user_workout };
        } else {
            return { "status": 2, "message": "user workouts not found" };
        }
    } catch (err) {
        return { "status": 0, "message": "Error occured while finding user workouts", "error": err }
    }
};
